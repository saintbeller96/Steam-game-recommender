...
if __name__ == "__main__":
    # dataset = []
    # data = pd.read_csv("groceries - groceries.csv", encoding='utf-8')
    # data = pd.DataFrame(data)
    # for i in data.index:
    #     val = data.loc[i]
    #     dataset.append(list(val.dropna())[1:])

    savePath = "MyModules/Results/fps_steam250.pickle"

    dbcon = MyDBController('127.0.0.1', 'guest', '0000', 'steam_db')
    user_list = dbcon.getUserList()
    dataset = [[game[0] for game in dbcon.getUserInfo(user)] for user in user_list]

    fpg = FP_Growth(200, savePath)#최소 지지도
    fpg.createTree(dataset=dataset)
    fpg.mine()

    # def printPatterns(filePath = "Results/result.pickle", num = 10, len_range = (1, 100)):
    #     pattern_dict = dict()
    #     with open(filePath, 'rb') as handle:
    #         pattern_dict = pickle.load(handle)
    #         del(pattern_dict["__header__"])

    #     tmp_dict = dict()
    #     for pattern, sup in pattern_dict.items():
    #         if(len(pattern) >= len_range[0] and len(pattern) < len_range[1]):
    #             tmp_dict[pattern] = sup

    #     tmp = sorted(tmp_dict.items(), key=lambda x:x[1], reverse = True)[:num]
    #     pprint(tmp)
   
    # printPatterns(filePath = savePath, len_range = (3, 5))
